...
from constants import WALL, FLOOR, PLAYER, BOX, TARGET, PLAYER_ON_TARGET, BOX_ON_TARGET
...
class Level:
    ...
    def get_state_string(self):
        """
        Get a string representation of the current level state.
        ...
        for y in range(self.height):
            row = ''.join(self.get_display_char(x, y) for x in range(self.width))
            rows.append(row)
        return '\n'.join(rows)
