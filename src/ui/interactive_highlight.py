...
class GameplayHighlight(InteractiveHighlight):
    ...
    def render_highlight(self, screen, map_area_x, map_area_y, cell_size, scroll_x=0, scroll_y=0):
        ...
        # Use different highlight intensity based on whether it's a valid move
        if self.show_movement_hints and self.is_adjacent_to_player():
            # Brighter highlight for valid moves
            highlight_color = (255, 255, 0, min(255, self.alpha + 64))
        else:
            # Normal highlight
            highlight_color = self.highlight_color
            
        # Create temporary surface with appropriate color
        ...
        # Draw the highlight
        screen.blit(temp_surface, (highlight_x, highlight_y))