...
Version optimisée avec les améliorations suivantes:
- Stratégies de sélection de cellules améliorées pour prioriser les états prometteurs
- Système de poids plus sophistiqué pour mieux guider la recherche
- Détection précoce de solution et de plateaux
- Mécanismes de cache pour éviter les calculs redondants
- Optimisation de la sélection des mouvements pour éviter l'explosion combinatoire

Optimisations implémentées:
- Détection avancée de deadlocks (corners, frozen boxes, line deadlocks)
- Filtrage intelligent des macro-moves pour réduire l'explosion combinatoire
- Heuristiques améliorées avec distances Manhattan vers les targets
- Système de poids adaptatif basé sur la progression
- Cache optimisé avec éviction LRU pour éviter les fuites mémoire
- Limitation intelligente de la génération de mouvements
"""
...
class DeadlockDetector:
    """
    Détecteur de deadlocks sophistiqué pour Sokoban.
    
    Détecte plusieurs types de deadlocks:
    - Corner deadlocks: box dans un coin sans target
    - Frozen box deadlocks: box bloquée de manière permanente
    - Line deadlocks: ligne de boxes contre un mur
    - Bipartite deadlocks: impossibilité de matcher boxes et targets
    """
    ...
    def __init__(self, level):
        ...
        # Précalcul des deadlock squares (positions impossibles)
        self.deadlock_squares = self._compute_deadlock_squares()
        
        # Cache pour éviter les recalculs
        self.deadlock_cache = {}
    
    def _compute_deadlock_squares(self) -> Set[Tuple[int, int]]:
        """Calcule toutes les positions qui sont des deadlocks évidents."""
        deadlocks = set()
        
        for y in range(self.height):
            for x in range(self.width):
                if not self.level.is_wall(x, y) and (x, y) not in self.targets:
                    if self._is_corner_deadlock_square(x, y):
                        deadlocks.add((x, y))
                    elif self._is_wall_adjacent_deadlock(x, y):
                        deadlocks.add((x, y))
        
        return deadlocks
    
    def _is_corner_deadlock_square(self, x: int, y: int) -> bool:
        """Vérifie si une position est un corner deadlock."""
        # Patterns de coins (2 murs adjacents)
        corner_patterns = [
            (self.level.is_wall(x-1, y) and self.level.is_wall(x, y-1)),  # Top-left
            (self.level.is_wall(x+1, y) and self.level.is_wall(x, y-1)),  # Top-right
            (self.level.is_wall(x-1, y) and self.level.is_wall(x, y+1)),  # Bottom-left
            (self.level.is_wall(x+1, y) and self.level.is_wall(x, y+1))   # Bottom-right
        ]
        return any(corner_patterns)
    
    def _is_wall_adjacent_deadlock(self, x: int, y: int) -> bool:
        """Vérifie si c'est un deadlock le long d'un mur."""
        # Si adjacent à un mur et pas de target dans la ligne
        adjacent_to_wall = (
            self.level.is_wall(x-1, y) or self.level.is_wall(x+1, y) or
            self.level.is_wall(x, y-1) or self.level.is_wall(x, y+1)
        )
        
        if not adjacent_to_wall:
            return False
            
        # Vérifier s'il y a un target accessible dans la direction du mur
        return not self._has_target_in_wall_direction(x, y)
    
    def _has_target_in_wall_direction(self, x: int, y: int) -> bool:
        """Vérifie s'il y a un target dans la direction du mur."""
        directions = [(0, 1), (1, 0), (0, -1), (-1, 0)]
        
        for dx, dy in directions:
            if self.level.is_wall(x + dx, y + dy):
                # Chercher des targets dans cette direction
                for i in range(1, max(self.width, self.height)):
                    tx, ty = x + i * dx, y + i * dy
                    if (0 <= tx < self.width and 0 <= ty < self.height):
                        if self.level.is_wall(tx, ty):
                            break
                        if (tx, ty) in self.targets:
                            return True
                    else:
                        break
        return False
    
    def is_deadlock(self, state: 'FESSState') -> bool:
        """
        Vérifie si un état est un deadlock.
        
        Retourne True si l'état est impossible à résoudre.
        """
        # Utiliser le cache si possible
        state_key = hash(state.boxes)
        if state_key in self.deadlock_cache:
            return self.deadlock_cache[state_key]
        
        # 1. Vérifier les deadlock squares évidents
        for box in state.boxes:
            if box in self.deadlock_squares:
                self.deadlock_cache[state_key] = True
                return True
        
        # 2. Vérifier les frozen boxes
        if self._has_frozen_boxes(state):
            self.deadlock_cache[state_key] = True
            return True
        
        # 3. Vérifier les line deadlocks
        if self._has_line_deadlock(state):
            self.deadlock_cache[state_key] = True
            return True
        
        # 4. Vérifier l'impossibilité de matching bipartite
        if self._is_bipartite_impossible(state):
            self.deadlock_cache[state_key] = True
            return True
        
        self.deadlock_cache[state_key] = False
        return False
    
    def _has_frozen_boxes(self, state: 'FESSState') -> bool:
        """Détecte les boxes qui ne peuvent plus bouger."""
        for box in state.boxes:
            if box not in self.targets:  # Les boxes sur target sont OK
                if self._is_box_frozen(box, state.boxes):
                    return True
        return False
    
    def _is_box_frozen(self, box: Tuple[int, int], all_boxes: FrozenSet[Tuple[int, int]]) -> bool:
        """Vérifie si une box est frozen (ne peut plus bouger)."""
        x, y = box
        
        # Compter les directions bloquées
        blocked_directions = 0
        
        for dx, dy in [(0, 1), (1, 0), (0, -1), (-1, 0)]:
            nx, ny = x + dx, y + dy
            
            # Direction bloquée si mur ou autre box
            if (self.level.is_wall(nx, ny) or (nx, ny) in all_boxes):
                blocked_directions += 1
        
        # Box frozen si bloquée dans au moins 3 directions
        return blocked_directions >= 3
    
    def _has_line_deadlock(self, state: 'FESSState') -> bool:
        """Détecte les deadlocks de ligne contre les murs."""
        # Vérifier les lignes horizontales
        if self._check_horizontal_line_deadlocks(state.boxes):
            return True
        
        # Vérifier les lignes verticales
        if self._check_vertical_line_deadlocks(state.boxes):
            return True
        
        return False
    
    def _check_horizontal_line_deadlocks(self, boxes: FrozenSet[Tuple[int, int]]) -> bool:
        """Vérifie les deadlocks de ligne horizontale."""
        # Grouper les boxes par ligne (y)
        boxes_by_row = defaultdict(list)
        for x, y in boxes:
            boxes_by_row[y].append(x)
        
        for y, x_positions in boxes_by_row.items():
            if len(x_positions) >= 2:  # Au moins 2 boxes sur la ligne
                x_positions.sort()
                
                # Vérifier si c'est une ligne continue contre un mur
                for i in range(len(x_positions) - 1):
                    if x_positions[i+1] - x_positions[i] == 1:  # Boxes adjacentes
                        # Vérifier s'il y a un mur au-dessus ou en-dessous
                        ...
                        if wall_above or wall_below:
                            # Vérifier s'il n'y a pas de targets dans cette ligne
                            targets_in_line = any((x, y) in self.targets for x in x_positions[i:i+2])
                            if not targets_in_line:
                                return True
        return False
    
    def _check_vertical_line_deadlocks(self, boxes: FrozenSet[Tuple[int, int]]) -> bool:
        """Vérifie les deadlocks de ligne verticale."""
        # Grouper les boxes par colonne (x)
        boxes_by_col = defaultdict(list)
        for x, y in boxes:
            boxes_by_col[x].append(y)
        
        for x, y_positions in boxes_by_col.items():
            if len(y_positions) >= 2:  # Au moins 2 boxes sur la colonne
                y_positions.sort()
                
                # Vérifier si c'est une ligne continue contre un mur
                for i in range(len(y_positions) - 1):
                    if y_positions[i+1] - y_positions[i] == 1:  # Boxes adjacentes
                        # Vérifier s'il y a un mur à gauche ou à droite
    ...
    def _is_bipartite_impossible(self, state: 'FESSState') -> bool:
        """Vérifie si le matching bipartite boxes-targets est impossible."""
        # Simplification: vérifier si toutes les boxes peuvent atteindre au moins un target
        for box in state.boxes:
            if box not in self.targets:  # Box pas encore sur un target
                if not self._box_can_reach_any_target(box, state.boxes):
                    return True
        return False
    
    def _box_can_reach_any_target(self, box: Tuple[int, int], all_boxes: FrozenSet[Tuple[int, int]]) -> bool:
        """Vérifie si une box peut atteindre au moins un target."""
        ...
        while queue:
            ...
            # Si on atteint un target, c'est bon
...
class FESSFeatureSpace:
    ...
    def get_next_cell_for_cycling(self) -> Optional[Tuple[int, int, int, int]]:
        ...
        Version optimisée qui priorise les cellules prometteuses:
        - Cellules avec plus de boxes packées (priorité maximale)
        - Cellules avec moins de boxes piégées
        - Cellules avec meilleure connectivité
        - Exploration périodique pour éviter les minimums locaux
        """
        if not self.non_empty_cells:
            return None

        # TODO: Ajouter un mécanisme de cache pour éviter de recalculer les cellules triées à chaque appel

        # Utiliser différentes stratégies selon le cycle pour éviter de rester coincé
        cycling_strategy = self.cell_cycle_index % 4  # Augmenté à 4 stratégies

        if cycling_strategy == 0:
            # Stratégie 1: Prioriser fortement les cellules avec plus de boxes packées (f1)
            # et moins de boxes piégées (f4)
            sorted_cells = sorted(self.non_empty_cells, 
                                 key=lambda cell: (-cell[0] * 2, cell[3], cell[2]))

            # Prendre la première cellule non vide
            for cell in sorted_cells:
                if self.fs_cells[cell]:
                    self.cell_cycle_index += 1
                    return cell

        elif cycling_strategy == 1:
            # Stratégie 2: Prioriser les cellules avec moins de boxes piégées (f4)
            # et plus de boxes packées (f1)
            sorted_cells = sorted(self.non_empty_cells, 
                                 key=lambda cell: (cell[3], -cell[0], cell[2]))
        ...
        elif cycling_strategy == 2:
            # Stratégie 3: Prioriser les cellules avec meilleure connectivité (f2)
            # et plus de boxes packées (f1)
            sorted_cells = sorted(self.non_empty_cells, 
                                 key=lambda cell: (cell[1], -cell[0], cell[3]))
            ...
            # Stratégie 4: Exploration aléatoire pour sortir des minimums locaux
            # Prendre une cellule au hasard parmi les 3 premières cellules triées par f1
            sorted_cells = sorted(self.non_empty_cells, key=lambda cell: -cell[0])
            candidates = sorted_cells[:min(3, len(sorted_cells))]

            if candidates:
                # Utiliser l'index comme pseudo-aléatoire
                cell = candidates[self.cell_cycle_index % len(candidates)]
                if self.fs_cells[cell]:
                    self.cell_cycle_index += 1
                    return cell

        # Fallback: cycling classique si aucune stratégie n'a fonctionné
        cell = self.non_empty_cells[self.cell_cycle_index % len(self.non_empty_cells)]
        self.cell_cycle_index = (self.cell_cycle_index + 1) % max(1, len(self.non_empty_cells))
        return cell
...
class FESSSearchEngine:
    ...
    def __init__(self, level, max_states=1000000, time_limit=120.0):
        ...
        self.move_cache = LRUCache(capacity=100000)
        self.state_evaluation_cache = LRUCache(capacity=50000)
        ...
        self.plateau_detection_threshold = 2000
        self.max_moves_per_state = 12  # Augmenté pour permettre plus d'exploration
        self.best_score_seen = -1
        
        # Désactiver temporairement les filtres trop agressifs pour les tests
    ...
    def search(self, progress_callback: Optional[Callable] = None) -> Optional[List[str]]:
        """
        Algorithme principal FESS selon Figure 2 du paper, avec optimisations.

        Cette version inclut:
        - Détection précoce de solution
        - Optimisation des poids des mouvements
        - Stratégies de sélection de cellules améliorées
        - Mécanismes pour éviter les minimums locaux
        ...
        best_packed_boxes = 0  # Pour suivre le progrès

        if progress_callback:
            progress_callback("🔬 Démarrage FESS authentique optimisé")

        # TODO: Ajouter une analyse préliminaire du niveau pour détecter les deadlocks évidents
        ...
        # Calculer les métriques initiales pour suivre le progrès
        initial_packed = self.feature_space.packing_analyzer.calculate_packing_feature(initial_state)
        best_packed_boxes = initial_packed
        total_boxes = len(self.level.boxes)
        ...
        # Variables pour détecter les plateaux (manque de progrès)
        plateau_counter = 0
        last_progress_states = self.states_explored

        # Search: while no solution has been found
        while self._within_limits():
            # Détection de plateau: si on a exploré beaucoup d'états sans amélioration
            if self.states_explored - last_progress_states > 1000:
                plateau_counter += 1
                last_progress_states = self.states_explored

                # TODO: Implémenter une stratégie de redémarrage ou de diversification
                # si on reste bloqué trop longtemps sur un plateau

                if plateau_counter > 5 and progress_callback:
                    progress_callback(f"⚠️ Détection de plateau: exploration sans progrès")
            ...
            # Vérification de deadlock avant d'ajouter l'état
            if self.deadlock_detector.is_deadlock(new_state):
                self.deadlocks_detected += 1
                continue  # État en deadlock, l'ignorer
            ...
            # Vérifier le progrès en termes de boxes packées
            new_packed = self.feature_space.packing_analyzer.calculate_packing_feature(new_state)
            if new_packed > best_packed_boxes:
                best_packed_boxes = new_packed
                plateau_counter = 0  # Réinitialiser le compteur de plateau
                last_progress_states = self.states_explored

                # Afficher le progrès significatif
                if progress_callback:
                    elapsed = time.time() - self.start_time
                    progress_callback(f"📦 Progrès: {best_packed_boxes}/{total_boxes} boxes rangées ({elapsed:.1f}s)")
            ...
            # Afficher le progrès périodiquement (mais pas trop souvent)
            current_time = time.time()
            if progress_callback and (self.states_explored % 1000 == 0 or 
                                     current_time - last_progress_time > 5.0):  # Max 1 message toutes les 5 secondes
        ...
        if progress_callback:
            ...
            progress_callback(f"📊 Meilleur résultat: {best_packed_boxes}/{total_boxes} boxes rangées")
    ...
    def _assign_move_weights(self, state: FESSState):
        ...
        - Priorité aux mouvements suggérés par les advisors
        """
        # Vérifier le cache d'évaluation
        state_key = hash(state.boxes)
        cached_weights = self.state_evaluation_cache.get(state_key)
        if cached_weights:
            for move, weight in cached_weights:
                state.add_child_move(move, weight)
            return

        moves_to_cache = []
        moves_added = 0

        # Étape 1: Obtenir les suggestions des advisors (priorité maximale)
        ...
        # Calculer les métriques pour l'état actuel (avec cache)
        current_metrics = self._get_state_metrics(state)
        current_packed_boxes, current_oop_boxes, current_connectivity = current_metrics

        # Étape 2: Ajouter les mouvements basiques avec filtrage intelligent
        basic_moves = ['UP', 'DOWN', 'LEFT', 'RIGHT']
        promising_moves = []
        
        for move in basic_moves:
            if moves_added >= self.max_moves_per_state:
                break
                
            # Vérifier rapidement si le mouvement est valide
            if not self._is_move_potentially_valid(state, move):
                continue
                
            # Simuler l'application du mouvement
            test_state = self._apply_basic_move(state, move, 0)
            if test_state:
                # Filtrage précoce: éviter les deadlocks évidents
                if self._is_obvious_deadlock_move(state, test_state):
                    continue
                
                # Calculer les métriques pour l'état résultant
                new_metrics = self._get_state_metrics(test_state)
                new_packed_boxes, new_oop_boxes, new_connectivity = new_metrics

                # Calculer le poids selon les priorités
                weight = self._calculate_move_weight(
                    current_packed_boxes, new_packed_boxes,
                    current_oop_boxes, new_oop_boxes,
                    current_connectivity, new_connectivity,
                    move in advisor_move_set
                )

                promising_moves.append((move, weight, new_packed_boxes - current_packed_boxes))

        # Trier par priorité (poids croissant, puis par amélioration de packing)
        promising_moves.sort(key=lambda x: (x[1], -x[2]))

        # Ajouter les meilleurs mouvements basiques (moins de filtrage pour les tests)
        move_limit = self.max_moves_per_state // 2 if self.enable_aggressive_filtering else len(promising_moves)
        for move, weight, _ in promising_moves[:move_limit]:
            state.add_child_move(move, weight)
            moves_to_cache.append((move, weight))
            moves_added += 1

        # Étape 3: Ajouter des macro-moves sélectionnés avec filtrage
        macro_move_limit = self.max_moves_per_state if self.enable_aggressive_filtering else self.max_moves_per_state * 2
        if moves_added < macro_move_limit:
            filtered_macro_moves = self._get_filtered_macro_moves(state, advisor_moves)
            
            for macro_move in filtered_macro_moves:
                if moves_added >= macro_move_limit:
                    break
                ...
                # Simuler l'application du macro-move
                test_state = self._apply_macro_move(state, move_id, 0)
                if test_state and not self._is_obvious_deadlock_move(state, test_state):
                    # Calculer les métriques pour l'état résultant
                    new_metrics = self._get_state_metrics(test_state)
                    new_packed_boxes, new_oop_boxes, new_connectivity = new_metrics
                    ...
                    state.add_child_move(move_id, weight)
                    moves_to_cache.append((move_id, weight))
                    moves_added += 1

        # Mettre en cache les résultats
        if moves_to_cache:
            self.state_evaluation_cache.put(state_key, moves_to_cache)
    ...
    def _find_least_weight_unexpanded_move(self, states: List[FESSState]) -> Optional[Tuple[FESSState, str, int]]:
        ...
        Version optimisée utilisant une file de priorité pour trouver plus efficacement
        le mouvement avec le poids minimum. Inclut également:
        - Limitation du nombre de mouvements par état pour éviter l'explosion combinatoire
        - Priorité aux mouvements avec poids négatifs (très prometteurs)
        - Détection et évitement des mouvements redondants

        Returns:
            Tuple[FESSState, str, int]: (état parent, mouvement, poids) ou None
        """
        # TODO: Ajouter un mécanisme de cache pour éviter de recalculer les mouvements pour des états similaires

        # Utiliser une file de priorité pour trouver efficacement le mouvement avec le poids minimum
        move_queue = []

        # File de priorité séparée pour les mouvements très prometteurs (poids négatifs)
        priority_moves = []

        # Pré-filtrer les états qui n'ont pas de mouvements non-explorés
        states_with_moves = [state for state in states if state.children_moves]

        # Si aucun état n'a de mouvements non-explorés, retourner None
        if not states_with_moves:
            return None

        # Nombre maximum de mouvements à considérer par état pour éviter l'explosion combinatoire
        # Pour les grands niveaux, limiter ce nombre peut considérablement améliorer les performances
        MAX_MOVES_PER_STATE = 10

        # Ajouter les mouvements à la file de priorité, en limitant le nombre par état
        for state in states_with_moves:
            # Trier les mouvements par poids croissant pour prendre les meilleurs
            sorted_moves = sorted(enumerate(state.children_moves), key=lambda x: x[1][1])

            # Limiter le nombre de mouvements par état
            for idx, (i, (move, move_weight)) in enumerate(sorted_moves[:MAX_MOVES_PER_STATE]):
                accumulated_weight = state.accumulated_weight + move_weight

                # Stocker (poids_accumulé, id_état, état, mouvement, poids_mouvement, index)
                move_entry = (accumulated_weight, id(state), state, move, move_weight, i)

                # Les mouvements avec poids négatifs sont très prometteurs (augmentent les boxes packées)
                # Les mettre dans une file de priorité séparée qui sera traitée en premier
                if move_weight < 0:
                    heapq.heappush(priority_moves, move_entry)
                else:
                    heapq.heappush(move_queue, move_entry)

        # Traiter d'abord les mouvements prioritaires (poids négatifs)
        if priority_moves:
            move_info = heapq.heappop(priority_moves)
        elif move_queue:  # Ensuite les mouvements normaux
            move_info = heapq.heappop(move_queue)
        else:
            return None  # Aucun mouvement disponible

        # Extraire les informations du mouvement sélectionné
        _, _, best_state, best_move, best_weight, best_move_index = move_info

        # Retirer le mouvement sélectionné de la liste pour éviter de le réexplorer
        if 0 <= best_move_index < len(best_state.children_moves):
            # Vérifier que le mouvement n'a pas été modifié entre-temps
            if best_state.children_moves[best_move_index][0] == best_move:
        ...
        return (best_state, best_move, best_weight)
    ...
    # Cache pour les mouvements basiques pour éviter les recalculs
    _move_cache = {}
    ...
    def _apply_basic_move(self, state: FESSState, move: str, weight: int) -> Optional[FESSState]:
        ...
        Version optimisée avec cache pour éviter les recalculs fréquents
        des mêmes mouvements dans des états similaires.
        """
        # Clé de cache: (position_joueur, hash_boxes, mouvement)
        cache_key = (state.player_pos, hash(state.boxes), move)

        # Vérifier si ce mouvement est déjà dans le cache
        if cache_key in self._move_cache:
            cached_result = self._move_cache[cache_key]
            if cached_result is None:
                return None

            # Créer une copie de l'état mis en cache avec le parent et le poids corrects
            return FESSState(
                player_pos=cached_result.player_pos,
                boxes=cached_result.boxes,
                parent=state,  # Utiliser le parent actuel
                move=move,
        ...
        if move not in directions:
            self._move_cache[cache_key] = None
            return None
        ...
        # Mettre en cache le résultat (sans le parent pour éviter les références circulaires)
        self._move_cache[cache_key] = new_state

        # Limiter la taille du cache pour éviter les fuites de mémoire
        if len(self._move_cache) > 100000:  # Ajuster selon les besoins
            # Vider 20% du cache quand il devient trop grand
            keys_to_remove = list(self._move_cache.keys())[:20000]
            for k in keys_to_remove:
                del self._move_cache[k]

        return new_state

    # Cache pour les macro-moves pour éviter les recalculs
    _macro_move_cache = {}
    ...
    def _apply_macro_move(self, state: FESSState, move_id: str, weight: int) -> Optional[FESSState]:
        ...
        Version optimisée avec cache pour éviter les recalculs fréquents
        des mêmes macro-moves dans des états similaires.
        """
        # Clé de cache: (hash_boxes, move_id)
        cache_key = (hash(state.boxes), move_id)

        # Vérifier si ce macro-move est déjà dans le cache
        if cache_key in self._macro_move_cache:
            cached_result = self._macro_move_cache[cache_key]
            if cached_result is None:
                return None
        ...
        try:
            ...
            if len(parts) < 2:
                self._macro_move_cache[cache_key] = None
                return None
            ...
            self._macro_move_cache[cache_key] = new_state

            # Limiter la taille du cache pour éviter les fuites de mémoire
            if len(self._macro_move_cache) > 50000:  # Ajuster selon les besoins
                # Vider 20% du cache quand il devient trop grand
                keys_to_remove = list(self._macro_move_cache.keys())[:10000]
                for k in keys_to_remove:
                    del self._macro_move_cache[k]
    ...
    def _calculate_move_weight(self, current_packed_boxes, new_packed_boxes,
                             current_oop_boxes, new_oop_boxes,
                             current_connectivity, new_connectivity,
                             is_advisor_move) -> int:
        """
        Calcule le poids d'un mouvement selon les priorités:
        1. Maximiser le nombre de caisses rangées (priorité absolue)
        2. Minimiser le nombre de corrals (caisses piégées)
        3. Minimiser l'heuristique précédente (connectivity)
        4. Favoriser les mouvements suggérés par les advisors

        Returns:
            int: Poids du mouvement (plus petit = meilleur)
        """
        # TODO: Ajouter un mécanisme de cache pour éviter de recalculer les poids pour des états similaires

        # Priorité 1: Maximiser le nombre de caisses rangées (priorité absolue)
        packed_boxes_diff = current_packed_boxes - new_packed_boxes

        # Si le mouvement augmente le nombre de caisses rangées, c'est excellent
        # Donner une priorité extrêmement élevée à ces mouvements
        if new_packed_boxes > current_packed_boxes:
            # Poids très négatif = priorité maximale
            # Plus la différence est grande, plus la priorité est élevée
            return -20 * (new_packed_boxes - current_packed_boxes)

        # Priorité 2: Minimiser le nombre de corrals (caisses piégées)
        oop_boxes_diff = new_oop_boxes - current_oop_boxes

        # Si le mouvement réduit significativement le nombre de caisses piégées
        if oop_boxes_diff < -1:  # Réduction de plus d'une caisse piégée
            return -5 * abs(oop_boxes_diff)  # Priorité élevée
        # Si le mouvement réduit le nombre de caisses piégées
        elif oop_boxes_diff < 0:
            return -3  # Priorité moyenne-haute

        # Priorité 3: Minimiser l'heuristique précédente (connectivity)
        connectivity_diff = new_connectivity - current_connectivity

        # Si le mouvement améliore significativement la connectivité
        if connectivity_diff < -1:
            return -2 * abs(connectivity_diff)  # Priorité moyenne
        # Si le mouvement améliore la connectivité
        elif connectivity_diff < 0:
            return 0  # Priorité standard

        # Si c'est un mouvement suggéré par un advisor, lui donner une priorité
        if is_advisor_move:
            # Les advisors sont des heuristiques spécialisées, leur faire confiance
            return 1

        # Pénaliser fortement les mouvements qui augmentent les corrals
        if oop_boxes_diff > 0:
            # Pénalité croissante avec le nombre de nouvelles caisses piégées
            return 10 + (oop_boxes_diff * 5)

        # Pénaliser les mouvements qui dégradent la connectivité
        if connectivity_diff > 0:
            # Pénalité croissante avec la dégradation de la connectivité
            return 5 + (connectivity_diff * 2)

        # Mouvement neutre - ni bon ni mauvais
        # Donner une priorité moyenne pour permettre l'exploration
        return 3

    def _get_state_metrics(self, state: FESSState) -> Tuple[int, int, int]:
        """Calcule les métriques d'un état avec mise en cache."""
        state_key = hash(state.boxes)
        cached_metrics = self.move_cache.get(f"metrics_{state_key}")
        
        if cached_metrics:
            return cached_metrics
        
        packed_boxes = self.feature_space.packing_analyzer.calculate_packing_feature(state)
        oop_boxes = self.feature_space.out_of_plan_analyzer.calculate_out_of_plan(state)
        connectivity = self.feature_space.connectivity_analyzer.calculate_connectivity(state)
        
        metrics = (packed_boxes, oop_boxes, connectivity)
        self.move_cache.put(f"metrics_{state_key}", metrics)
        return metrics

    def _is_move_potentially_valid(self, state: FESSState, move: str) -> bool:
        """Vérification rapide de validité d'un mouvement sans simulation complète."""
        player_x, player_y = state.player_pos
        ...
        # Vérification basique des limites et murs
        if (new_x < 0 or new_x >= self.level.width or
            new_y < 0 or new_y >= self.level.height or
            self.level.is_wall(new_x, new_y)):
            return False
    ...
    def _is_obvious_deadlock_move(self, original_state: FESSState, new_state: FESSState) -> bool:
        """Détection rapide de mouvements qui mènent à des deadlocks évidents."""
        # Vérifier si une box a été poussée dans un deadlock square
        box_moved = original_state.boxes != new_state.boxes
        
        if box_moved:
            new_boxes = new_state.boxes - original_state.boxes
            for new_box in new_boxes:
                if new_box in self.deadlock_detector.deadlock_squares:
                    return True
                    
        return False

    def _get_filtered_macro_moves(self, state: FESSState, advisor_moves) -> List:
        """Génère des macro-moves filtrés pour éviter l'explosion combinatoire."""
        all_macro_moves = self.macro_generator.generate_macro_moves(state)
        
        # Filtrer par distance et priorité
        filtered_moves = []
        advisor_targets = set()
        
        # Identifier les targets suggérés par les advisors
        for macro_move, _ in advisor_moves:
            if macro_move:
                advisor_targets.add(macro_move.box_to)
        
        for macro_move in all_macro_moves:
            # Priorité 1: Mouvements vers des targets
            if macro_move.box_to in self.level.targets:
                filtered_moves.append(macro_move)
            # Priorité 2: Mouvements suggérés par les advisors
            elif macro_move.box_to in advisor_targets:
                filtered_moves.append(macro_move)
            # Priorité 3: Mouvements courts (distance <= 3)
            elif (abs(macro_move.box_to[0] - macro_move.box_from[0]) +
                  abs(macro_move.box_to[1] - macro_move.box_from[1])) <= 3:
                filtered_moves.append(macro_move)
        
        # Limiter le nombre total de macro-moves
        return filtered_moves[:self.max_moves_per_state // 2]
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        return {
            'algorithm': 'FESS_AUTHENTIC_ENHANCED',
            'search_statistics': {
            ...
            'optimization_statistics': {
                'deadlock_squares': len(self.deadlock_detector.deadlock_squares),
                'move_cache_size': len(self.move_cache.cache),
                'state_evaluation_cache_size': len(self.state_evaluation_cache.cache),
                'max_moves_per_state': self.max_moves_per_state,
