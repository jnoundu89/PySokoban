...
from src.ui.skins.skin_manager import SkinManager
    ...
    def _draw_editor(self):
        ...
        for y in range(self.current_level.height):
            for x in range(self.current_level.width):
                ...
                # Draw cell
                if cell == WALL:
                    self.screen.blit(self.skin_manager.wall_img, (cell_x, cell_y))
                elif cell == FLOOR:
                    self.screen.blit(self.skin_manager.floor_img, (cell_x, cell_y))
                elif cell == PLAYER:
                    self.screen.blit(self.skin_manager.floor_img, (cell_x, cell_y))
                    self.screen.blit(self.skin_manager.player_img, (cell_x, cell_y))
                elif cell == BOX:
                    self.screen.blit(self.skin_manager.floor_img, (cell_x, cell_y))
                    self.screen.blit(self.skin_manager.box_img, (cell_x, cell_y))
                elif cell == TARGET:
                    self.screen.blit(self.skin_manager.floor_img, (cell_x, cell_y))
                    self.screen.blit(self.skin_manager.target_img, (cell_x, cell_y))
                elif cell == PLAYER_ON_TARGET:
                    self.screen.blit(self.skin_manager.floor_img, (cell_x, cell_y))
                    self.screen.blit(self.skin_manager.target_img, (cell_x, cell_y))
                    self.screen.blit(self.skin_manager.player_img, (cell_x, cell_y))
        ...
        for i, element in enumerate(self.palette_elements):
            ...
            # Draw element
            if element == WALL:
                self.screen.blit(self.skin_manager.wall_img, rect)
            elif element == FLOOR:
                self.screen.blit(self.skin_manager.floor_img, rect)
            elif element == PLAYER:
                self.screen.blit(self.skin_manager.floor_img, rect)
                self.screen.blit(self.skin_manager.player_img, rect)
            elif element == BOX:
                self.screen.blit(self.skin_manager.floor_img, rect)
                self.screen.blit(self.skin_manager.box_img, rect)
            elif element == TARGET:
                self.screen.blit(self.skin_manager.floor_img, rect)
                self.screen.blit(self.skin_manager.target_img, rect)
